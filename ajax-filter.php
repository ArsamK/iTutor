...
// Perform any operations you need with $subject or other values
...
$sql = "SELECT DISTINCT teacher.* FROM teacher";

// Initialize an array to hold the conditions
$conditions = [];

// Check each filter and add conditions as needed
if (!empty($subjectName) && $subjectName != "select subject") {
    // Join with the subject table to filter by subject name
    $sql .= " JOIN subject ON teacher.subjectid = subject.subjectid";
    $conditions[] = "subject.subjectName = '$subjectName'";
}
if (!empty($language)  && $language != "Also speaks") {
    // Join with the teacherlanguage table to filter by language
    $sql .= " JOIN teacherlanguage ON teacher.teacherid = teacherlanguage.teacherid";
    $conditions[] = "teacherlanguage.language = '$language'";
}
if (!empty($day)  && $day != "Any day") {
    // Join with the teacheravailability table to filter by day
    $sql .= " JOIN teacheravailability ON teacher.teacherid = teacheravailability.teacherid";
    $conditions[] = "teacheravailability.day = '$day'";
}
if (!empty($price)  && $price != "5000") {
    $conditions[] = "teacher.rates <= $price"; // Assuming rates is a numeric value
}
if (!empty($city)  && $city != "City") {
    $conditions[] = "teacher.city = '$city'";
}
...
// Add sorting based on high price and low price
if ($sortBy == 'high price') {
  $sql .= " ORDER BY teacher.rates DESC";
} elseif ($sortBy == 'low price') {
  $sql .= " ORDER BY teacher.rates ASC";
} else{
  $sql .= " ORDER BY teacher.teacherid ASC";
}

// Add the ORDER BY, LIMIT, etc., as needed
$sql .= " LIMIT $offset, $limit";

// Now, $sql contains the dynamically generated query with DISTINCT based on teacherid
